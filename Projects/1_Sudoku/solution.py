...
def naked_twins(values):
    ...
    for boxA in output:
        for boxB in peers[boxA]:
            if output[boxA] == output[boxB] and len(output[boxA]) == 2:
                if boxB not in pairs:
                    pairs[boxA] = boxB

    for boxA, boxB in pairs.items():
        for peer in [value for value in peers[boxA] if value in peers[boxB]]:
